...
S=[{"tr":sec["title"]["tr"],"g":[{"tr":g["title"]["tr"],"i":[{"n":it["name"]["tr"],"ne":it["name"]["en"],"p":it["p"],"ing":it["ing"]["tr"],"inge":it["ing"]["en"],"a":it["a"],"kcal":it.get("kcal"),"nut":it.get("nut"),"alc":it.get("alc",0),"pork":it.get("pork",0),"sec_id":sec["id"]} for it in g["i"]]} for g in sec["g"]]} for sec in ST["sections"]]
...
hdr = ["Bölüm","Grup","Ürün (TR)","Ürün (EN)","Fiyat ₺","İçindekiler (TR)","İçindekiler (EN)"] + CODES + ["Enerji kcal (porsiyon)","Protein g","Yağ g","Karbonhidrat g","Alkol (X)","Domuz türevi (X)","Notunuz"]
ws.append(hdr)
...
for s in S:
    for g in s["g"]:
        for it in g["i"]:
            ...
            nu = it.get("nut") or {}
            row += [("" if it.get("kcal") is None else it["kcal"]), nu.get("p",""), nu.get("f",""), nu.get("c",""), ("X" if it.get("alc") else ""), ("X" if it.get("pork") else ""), ""]
            ws.append(row)
            for c in range(1, len(hdr)+1):
...
widths = [15,24,32,32,9,52,52] + [5]*len(CODES) + [12,9,8,13,9,11,26]
